...
//cctype - isdigit(), isalpha(), isspace()
#include <cctype>
using namespace std;

int main(int argc, char*  argv[]) {

    // Case 3, file name wasn't listed as an argument to main.
    if (argc == 1) {
        cout << "argc " << argc <<endl;
        return 0;
    }
    ...
    // THE BELOW CODE WON'T HAPPEN DURING CASE 3.



    // --Main Variables for iofstream--
    string file_name = argv[1]; //"myfile4.txt" // Set string to the name of the filename, which is argv[1]
    ifstream finObj;
    ...
    if (argc >= 1) {
        // Open the file with the istream object finObj
        finObj.open(file_name);
        ...
        // This is for myfile3 INPUT case where the file name is not there to be read. argc = 1
    }
    ...
    //code to read no file is provided. CASE 3
